...
class DailyPrice():
    ...
    def dataBasePgsConn(host='localhost', database='market', user='postgres', password='1234', cursor_factory=RealDictCursor):
        conn = psycopg2.connect(host=host, database=database, user=user,
                                password=password, cursor_factory=cursor_factory)
        return conn
    ...
    def bavcopyDate(date=dt.date.today(), delta=0):
        return (date-dt.timedelta(delta)).strftime('%d%b%Y').upper()

    def bhavcopyUrl(dateFormated):
        # return f'https://www1.nseindia.com/content/historical/EQUITIES/2022/{dateFormated[2:5]}/cm{{dateFormated}}bhav.csv.zip'
        return f'https://archives.nseindia.com/content/historical/EQUITIES/{dateFormated[5:]}/{dateFormated[2:5]}/cm{dateFormated}bhav.csv.zip'
        # return f'https://www1.nseindia.com/content/historical/EQUITIES/{dateFormated[5:]}/{dateFormated[2:5]}/cm{dateFormated}bhav.csv.zip'

    def rbi_dict(c, date=dt.date.today(), delta=0):
        rbi_ref = nsepy.get_rbi_ref_history(
            (date-dt.timedelta(delta)), (date-dt.timedelta(delta)))
        print(rbi_ref)
    ...
    def rbiPgs(c, conn, rbi_ref, cur_dict, delta=0):

        for i in range(len(rbi_ref.T)):
            register_adapter(np.int64, psycopg2._psycopg.AsIs)
            date = str((dt.datetime.today()-dt.timedelta(delta)).date())
    ...
    def rbiSqlite(c, conn, rbi_ref, cur_dict, delta=0):
    ...
    def metal_sqlite(cu_df, c, conn):
        ...
        for i in range(len(cu_df)):
            ...
            c.execute("INSERT INTO cu_lme_csp('com_id','date','Cu_CSP','Cu_Fut','Cu_Stock' ) VALUES ( ?,?,?,?,?)",
                      (com_val[i], date, cu_df['csp'].iloc[i], cu_df['fut'].iloc[i], cu_df['stock'].iloc[i]))
            conn.commit()
    ...
    def cu_pgs(cu_df, c, conn):
        ...
        for i in range(len(cu_df)):
            ...
            c.execute("""INSERT INTO cu_lme_csp("com_id","date","Cu_CSP","Cu_Fut","Cu_Stock") VALUES ( %s,%s,%s,%s,%s)""",
                      (com_val[i], date, cu_df['csp'].iloc[i], cu_df['fut'].iloc[i], cu_df['stock'].iloc[i]))
        conn.commit()
    ...
    def bhavcopy(bhav_url, folder_location):
        ...
        # avoiding the '.zip'(last 4 charcters) extension
    ...
    def stockData(stock_df, c):
        ...
        stock_data = pd.DataFrame(
            stock_dict.items(), columns=['SYMBOL', 'stk_id'])
        ...
        # final_df = (pd.merge(daily_data, stock_data)
        #             .sort_values('stk_id', inplace=True)
    ...
    def indexdate(date=dt.date.today(), delta=0):
        return (date-dt.timedelta(delta)).strftime('%d%m%Y').upper()

    def index_url(indexdateformatted):
        index_url = f'https://archives.nseindia.com/content/indices/ind_close_all_{indexdateformatted}.csv'
        return index_url

    def indexDf(index_url, folder_location):
